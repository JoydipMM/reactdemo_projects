...
// create context for ai, which helps to remember the previous conversation and use it to generate response
...
async function aiAnswer (user_input) {
    ...
    // first push our user input to chatMemoryContext
    chatMemoryContext.push({
        role:"user",
        content:user_input
    })

    const response = await AIClient.responses.create({
        input: chatMemoryContext, // we will pass the chatMemoryContext as input where ai will remember the previous conversation
        model: aiModel,
    });

    // now store the response in chatMemoryContext
    chatMemoryContext.push({
        role:"assistant",
