...
import innovationsstyles from "@/components/manufacturingUnit/innovations/innovations.module.css";

export default function Innovations({ innovationsData }) {
  const allInnovationsData = innovationsData?.data?.content;
  const allInnovations = allInnovationsData?.innovations;
      ...
      >
        <div className="container">
          <div className={precationstyles.whydecor_wrp}>
            <div
              ref={mainSectionRef}
              className={`${precationstyles.whydecorleft} sticky-section ${innovationsstyles.innovl}`}
            >
              <h2>
                <AnimatedText text={allInnovationsData?.title} />
              </h2>

              {allInnovationsData?.description}
              <div></div>
            </div>
            <div className={precationstyles.whydecorrit}>
              <div className="sections-container">
                {allInnovations.map((allInnovation) => {
                  count++;
                  return (
                    <div
                    ...
                    >
                      <div className={precationstyles.prodmwrper}>
                        <div className={precationstyles.prodimcont}>
                          ...
                          <Image
                            src={allInnovation?.image?.image_url}
                            alt="image"
                            fill={true}
                          />
                        </div>
                        <h4>{allInnovation?.heading}</h4>
                        <p>{allInnovation?.description}</p>
                      </div>
                    </div>
                  );
                })}
