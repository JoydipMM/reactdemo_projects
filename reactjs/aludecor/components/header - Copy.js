          ...
          <div className={styles.secondmenubar}>
            ...
            <div className="right-part">
              <div className="ruby-menu-demo-header">
                <div className="ruby-wrapper">
                  <ul className="ruby-menu">
                    {allPrimaryMenus.map((allPrimaryMenu, index) => {
                      const childCount = allPrimaryMenu?.children;
                      const totalchildCount = childCount.length;
                      // className = "ruby-menu-mega-blog dropdownch";
                      return (
                        <li
                          className={
                            totalchildCount > 0
                              ? "ruby-menu-mega-blog dropdownch"
                              : "ruby-menu-mega-blog"
                          }
                          key={`menu-${index}`}
                        >
                          <Link href={allPrimaryMenu.url || "#"}>
                            {allPrimaryMenu.title || "Untitled"}
                          </Link>
                          {totalchildCount > 0 ? (
                            <>
                              ...
                              <div className="main-wrap">
                                <ul className="ruby-menu-mega-blog-nav">
                                  {/* Child Inner Menu Start*/}
                                  {allPrimaryMenu?.children
                                    ? allPrimaryMenu?.children.map(
                                        (allChildMenu, index) => {
                                          const innerChilds =
                                            allChildMenu?.children;
                                          const totalInnerChildCount =
                                            innerChilds.length;
                                          return (
                                            <li
                                              className={
              ...
              <div className="mobmenu">
                <MobMenu pMenu={primaryMenu} sMenu={secondaryMenu} />
              </div>
              <Link
