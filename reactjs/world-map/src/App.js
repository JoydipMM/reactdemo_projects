...
export default function App() {
  ...
  const earthGroup = earthGroupRef.current; // ✅ Add this line
  const { lon } = selectedContinent;
  const { lat } = selectedContinent; // or directly use it inline below

  const targetRotation = {
    x: THREE.MathUtils.degToRad(lat), // Maintain axial tilt
    y: THREE.MathUtils.degToRad(lon), // Rotate globe to bring longitude to front
    z: 0,
  };
  ...
  const startRotation = {
    x: earthGroup.rotation.x,
    y: earthGroup.rotation.y,
    z: earthGroup.rotation.z,
  };

  const animate = () => {
    frame++;
    const t = frame / duration;
    const easedT = t * (2 - t); // Ease-out interpolation

    earthGroup.rotation.x = startRotation.x + (targetRotation.x - startRotation.x) * easedT;
    earthGroup.rotation.y = startRotation.y + (targetRotation.y - startRotation.y) * easedT;
    earthGroup.rotation.z = startRotation.z + (targetRotation.z - startRotation.z) * easedT;

    console.log(`x- ${earthGroup.rotation.x} --- y- ${earthGroup.rotation.y}`)
